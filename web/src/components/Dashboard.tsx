...
function StatCard({ label, value, sub, color, icon }: { label: string; value: string | number; sub?: string; color: string; icon: string }) {
  return (
    <div className={`relative overflow-hidden rounded-2xl border ${color} p-5`}>
      ...
      <div className="absolute -bottom-4 -right-4 w-24 h-24 rounded-full opacity-5 bg-white"></div>
    </div>
  );
}
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-xs text-gray-500">{count} ({pct.toFixed(1)}%)</span>
        </div>
          ...
          <div key={idx} className="group relative">
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 h-5 rounded bg-white/5 overflow-hidden relative">
                ...
                <span className="absolute inset-0 flex items-center px-2 text-[10px] font-mono text-white/70 truncate">
                  {endpoint}
                </span>
              </div>
            ...
            <div className="absolute bottom-full left-10 mb-1 hidden group-hover:block z-10">
              <div className="bg-[#1a1a2e] border border-white/10 rounded-lg px-3 py-2 text-xs shadow-xl max-w-sm">
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b border-white/5">
            <th className="py-2 px-3 text-left text-gray-500 font-medium">Endpoint</th>
            <th className="py-2 px-3 text-right text-gray-500 font-medium">Calls</th>
            <th className="py-2 px-3 text-right text-gray-500 font-medium">Avg Time</th>
            <th className="py-2 px-3 text-right text-gray-500 font-medium">Errors</th>
            <th className="py-2 px-3 text-right text-gray-500 font-medium">Performance</th>
          </tr>
        </thead>
        <tbody>
          {sorted.map((ep, idx) => {
            ...
            return (
              <tr key={idx} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                <td className="py-2.5 px-3">
                  ...
                  <span className="block text-[10px] text-gray-600 mt-0.5 break-all">{ep.path}</span>
                </td>
                <td className="py-2.5 px-3 text-right text-white font-medium">{ep.count}</td>
    ...
    <div className="flex flex-wrap gap-2">
      {events.map((evt, idx) => (
        ...
        >
          ...
          <span className="text-[10px] text-gray-400">{stateLabels[evt.state] || evt.state}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
        <StatCard label="Total Requests" value={stats.totalRequests} icon="📤" color="border-blue-500/20 bg-blue-500/5" />
        <StatCard label="Total Responses" value={stats.totalResponses} icon="📥" color="border-emerald-500/20 bg-emerald-500/5" />
        <StatCard label="Errors" value={stats.totalErrors} icon="❌" color="border-red-500/20 bg-red-500/5" sub={`${stats.errorRate}% error rate`} />
        <StatCard label="Success Rate" value={`${successRate}%`} icon="✅" color="border-emerald-500/20 bg-emerald-500/5" />
        <StatCard label="Avg Response" value={stats.avgResponseTime >= 1000 ? `${(stats.avgResponseTime / 1000).toFixed(1)}s` : `${stats.avgResponseTime}ms`} icon="⚡" color="border-yellow-500/20 bg-yellow-500/5" sub={`Max: ${(stats.maxResponseTime / 1000).toFixed(1)}s`} />
        <StatCard label="Unique Endpoints" value={Object.keys(stats.endpoints).length} icon="🔗" color="border-purple-500/20 bg-purple-500/5" sub={`${requestEntries.length} total calls`} />
      </div>
      ...
      {stats.lifecycleEvents.length > 0 && (
        <div className="rounded-2xl border border-white/5 bg-[#12121a] p-5">
          <h3 className="text-sm font-semibold text-gray-300 mb-4 flex items-center gap-2">
            ...
            <span className="text-xs text-gray-600">({stats.lifecycleEvents.length} transitions)</span>
          </h3>
      ...
      <div className="rounded-2xl border border-white/5 bg-[#12121a] p-5">
        <h3 className="text-sm font-semibold text-gray-300 mb-4 flex items-center gap-2">
          ...
          Endpoint Performance
          <span className="text-xs text-gray-600">({Object.keys(stats.endpoints).length} endpoints)</span>
        </h3>
        ...
        <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-5">
          ...
          <div className="space-y-3">
            {errorEntries.map((err) => (
              <div key={err.id} className="rounded-xl bg-[#12121a] border border-red-500/10 p-4">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    {err.responseTime && (
                      <span className="text-xs text-gray-500 ml-2">{err.responseTime}ms</span>
                    )}
                  </div>
                </div>
                {err.url && (
                  <p className="text-xs font-mono text-gray-400 mt-2 break-all">{err.url}</p>
                )}
                {err.errorType && (
                  <p className="text-xs text-orange-400 mt-1">{err.errorType}</p>
                )}
                {err.errorMessage && (
                  <pre className="text-xs text-gray-500 mt-2 whitespace-pre-wrap font-mono bg-black/20 rounded-lg p-3">
                    {err.errorMessage}
                  </pre>
                )}
                {err.body && (
                  <pre className="text-xs text-gray-400 mt-2 whitespace-pre-wrap font-mono bg-black/20 rounded-lg p-3 max-h-40 overflow-auto">
                    {err.body}
                  </pre>
                )}
              </div>
